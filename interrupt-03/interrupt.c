 ...
 * @Last modified time: 2018-10-23T11:02:24+08:00
 * @License: GPL
 */
#include "interrupt.h"
...
void key_work_func(struct work_struct *work) {
  ...
  IRQ_ST *k = container_of(work, IRQ_ST, work);
...
void key_delay_work_func(struct work_struct *work) {
  ...
  IRQ_ST *k = container_of(d, IRQ_ST, delay_work);
...
static __init int interrupt_init(void) {
  ...
  INIT_WORK(&irq_key.work, key_work_func);
  INIT_DELAYED_WORK(&irq_key.delay_work, key_delay_work_func);
...
static __exit void interrupt_exit(void) {
  ...
  flush_delayed_work(&irq_key.delay_work);
  /*结束添加*/
  PERR("EXIT\n");
  return;
}
